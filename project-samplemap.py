...
class Example(QMainWindow):
    ...
    def __init__(self):
        ...
        # with open('./Data/TL_SCCO_SIG_WGS84.json, encoding="utf-8"') as f:
        ...
        # import folium
        # import webbrowser
        # state_geo = './Data/Folium_Test/TL_SCCO_SIG_WGS84.json'
        # state_apart = './Data/Folium_Test/korea_apart_decoded2.csv'
        # state_data = pd.read_csv(state_apart, encoding = 'utf-8')
        # m = folium.Map(location=[36, 127],  zoom_start=7)
        # m.choropleth(
        # geo_data=state_geo,
        ...
        g1 = folium.plugins.FeatureGroupSubGroup(mcg, '피자헛')
        m.add_child(g1)
        g2 = folium.plugins.FeatureGroupSubGroup(mcg, '피자스쿨')
        m.add_child(g2)
        g3 = folium.plugins.FeatureGroupSubGroup(mcg, '도미노피자')
        m.add_child(g3)
        g4 = folium.plugins.FeatureGroupSubGroup(mcg, '미스터피자')
        m.add_child(g4)
        g5 = folium.plugins.FeatureGroupSubGroup(m, 'K-Means 클러스터')
        m.add_child(g5)


        # 마켓클러스터 사용안하는거 (쫌 많이 느림)
        # g1 = folium.plugins.FeatureGroupSubGroup(m, '피자헛')
        # m.add_child(g1)
        # g2 = folium.plugins.FeatureGroupSubGroup(m,'피자스쿨')
        # m.add_child(g2)
        # g3 = folium.plugins.FeatureGroupSubGroup(m,'도미노피자')
        # m.add_child(g3)
        # g4 = folium.plugins.FeatureGroupSubGroup(m,'미스터피자')
        # m.add_child(g4)
        # g5 = folium.plugins.FeatureGroupSubGroup(m,'K-Means 클러스터')
        # m.add_child(g5)
        ...
        df4 = pd.read_csv("./Data/mrpizza_complete.csv")
        ...
        for n in df4.index:
            folium.Marker([df4['lat'][n], df4['lng'][n]],icon=folium.Icon(color='green', icon='info-sign')).add_to(g4)
        ...
        # iframe = branca.element.IFrame(html=f, width=500, height=300)
        # popup = folium.Popup(iframe, max_width=500)
        # fastestRoute.add_child(popup)


        # self.us.save("./Data/test.html")
